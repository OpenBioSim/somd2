...
# Energy threshold (kcal/mol) for the "active" state: alpha=0 should give
...
# ── schedule alpha/coul_kappa tests ───────────────────────────────────────────
#
# These tests verify alpha and coul_kappa values by calling schedule.morph()
# directly, using the same initial/final values that lambdalever passes in
# production. They are completely independent of Sire's energy formula and will
# continue to work correctly regardless of changes to the softcore
# implementation.

# ring_break_morph alpha points (3 equal stages: [0,1/3), [1/3,2/3), [2/3,1]):
#   λ=0.00  potential_swap start     alpha=1
#   λ=0.15  potential_swap mid       alpha=1
#   λ=1/3   restraints_off start     alpha=1     (within-stage lam=0)
#   λ=0.45  restraints_off mid       alpha=0.65  (within-stage lam=0.35)
#   λ=0.50  restraints_off mid       alpha=0.5   (within-stage lam=0.5)
#   λ=0.55  restraints_off mid       alpha=0.35  (within-stage lam=0.65)
#   λ=0.60  restraints_off near end  alpha=0.2   (within-stage lam=0.8)
#   λ=2/3   morph start              alpha=0
#   λ=0.85  morph mid                alpha=0
#   λ=1.00  morph end                alpha=0
_FWD_ALPHA = [
    (0.00, 1.0),
    (0.15, 1.0),
    (1 / 3, 1.0),
...
# reverse_ring_break_morph ring-make alpha points (mirror of forward):
#   λ=0.00  reversed morph start      alpha=0
#   λ=0.15  reversed morph mid        alpha=0
#   λ=1/3   reversed restraints_off start  alpha=0  (within-stage lam=0)
#   λ=0.45  reversed restraints_off mid    alpha=0.35
#   λ=0.50  reversed restraints_off mid    alpha=0.5
#   λ=0.55  reversed restraints_off mid    alpha=0.65
#   λ=0.60  reversed restraints_off near end  alpha=0.8
#   λ=2/3   reversed potential_swap start  alpha=1
#   λ=0.85  reversed potential_swap mid    alpha=1
#   λ=1.00  reversed potential_swap end    alpha=1
_REV_ALPHA = [
    (0.00, 0.0),
    (0.15, 0.0),
    (1 / 3, 0.0),
...
@pytest.mark.parametrize("lam,expected_alpha", _FWD_ALPHA)
def test_ring_break_morph_schedule(lam, expected_alpha):
    """
    ring_break_morph() produces the correct ring-break alpha at each λ.

    Uses lambdalever's initial/final values (alpha: 1→0) to ensure the test
...
@pytest.mark.parametrize("lam,expected_alpha", _REV_ALPHA)
def test_reverse_ring_break_morph_schedule(lam, expected_alpha):
    """
    reverse_ring_break_morph() produces the correct ring-make alpha at each λ.

    Uses lambdalever's initial/final values (alpha: 0→1) to ensure the test
    matches production behaviour exactly.
    """
    from somd2._utils._schedules import reverse_ring_break_morph
...
def test_ring_break_active_in_morph(forward_dynamics, lam):
    """
    Ring-break energy is clearly non-zero (alpha=0) throughout the morph stage.
    """
    ...
    assert abs(e) > _ACTIVE_THRESHOLD, (
        ...
        f"threshold {_ACTIVE_THRESHOLD} kcal/mol (alpha should be 0)"
    )


def test_ring_make_active_at_lambda_zero(reverse_dynamics):
    """
    Ring-make energy is non-zero at λ=0: the morph stage fixes alpha=0
    ...
    assert abs(e) > _ACTIVE_THRESHOLD, (
        ...
        f"{_ACTIVE_THRESHOLD} kcal/mol (alpha should be 0 in morph stage)"
    )


def test_ring_make_inactive_at_lambda_one(reverse_dynamics):
    """
    Ring-make energy is near-zero at λ=1 (potential_swap end, alpha=1).
    ...
    assert abs(e) < _ACTIVE_THRESHOLD, (
        ...
        f"{_ACTIVE_THRESHOLD} kcal/mol (alpha should be 1)"
    )
...
# that the softcore alpha values at (forward, λ) and (reverse, 1-λ) are equal.
# Both forces act on the same bond (the original ring_breaking_bond, which
# swap_end_states=True maps to ring_making_pairs), so the energies must also
# match.  The hard-hard correction appears identically on both sides and
# cancels in the comparison, making this test robust to formula changes.
#
# Test points span zero and non-zero energy regions:
#   λ=0.0  → forward alpha=1, reverse at 1-λ=1.0 alpha=1 (both ≈0)
#   λ=0.55 → forward restraints_off (alpha=0.35), reverse restraints_off at 0.45 (alpha=0.35)
#   λ=2/3  → forward morph start (alpha=0), reverse restraints_off start at 1/3 (alpha=0)
#   λ=0.85 → forward morph (alpha=0), reverse reversed-morph at 0.15 (alpha=0)
#   λ=1.0  → forward morph end (alpha=0), reverse at 0.0 reversed-morph (alpha=0)
...
def test_energy_symmetry_forward_reverse(forward_dynamics, reverse_dynamics, lam):
    ...
    that the mirrored alpha values produce identical corrections on the same bond.
    """
...
def test_schedule_symmetry():
    ...
    comparing lever values at a dense grid of lambda points using the default
    initial/final values that lambdalever passes for each lever.
    """
